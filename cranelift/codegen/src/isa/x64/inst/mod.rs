...
pub(crate) enum Inst {
    ...
    /// mov (64 32) reg reg
    Mov_R_R { is_64: bool, src: Reg, dst: Reg },
...
impl Inst {
    ...
    pub(crate) fn alu_rmi_r(is_64: bool, op: RMI_R_Op, src: RMI, wdst: Writable<Reg>) -> Self {
        let dst = wdst.to_reg();
    ...
    pub(crate) fn imm_r(dst_is_64: bool, simm64: u64, wdst: Writable<Reg>) -> Inst {
        let dst = wdst.to_reg();
        debug_assert!(dst.get_class() == RegClass::I64);
        if !dst_is_64 {
    ...
    pub(crate) fn mov64_m_r(addr: Addr, wdst: Writable<Reg>) -> Inst {
        let dst = wdst.to_reg();
        debug_assert!(dst.get_class() == RegClass::I64);
        Inst::Mov64_M_R { addr, dst }
    }

    pub(crate) fn movsx_m_r(extMode: ExtMode, addr: Addr, wdst: Writable<Reg>) -> Inst {
        let dst = wdst.to_reg();
        debug_assert!(dst.get_class() == RegClass::I64);
    ...
    pub(crate) fn shift_r(
        ...
        let dst = wdst.to_reg();
        debug_assert!(nBits < if is_64 { 64 } else { 32 });
        debug_assert!(dst.get_class() == RegClass::I64);
        Inst::Shift_R {
            is_64,
            kind,
    ...
    pub(crate) fn pop64(wdst: Writable<Reg>) -> Inst {
...
impl MachInst for Inst {
    ...
    fn is_move(&self) -> Option<(Writable<Reg>, Reg)> {
        ...
        // %reg.
        match self {
            Self::Mov_R_R { is_64, src, dst } if *is_64 => Some((Writable::from_reg(*dst), *src)),
            _ => None,
        }
    }
