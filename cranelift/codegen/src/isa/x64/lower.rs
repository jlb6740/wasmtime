...
fn lower_insn_to_regs<C: LowerCtx<I = Inst>>(
    ...
    match op {
        ...
        Opcode::Vpopcnt => {
            ...
            ctx.emit(Inst::xmm_rm_r(
                SseOpcode::Pxor,
                RegMem::from(low_mask),
                low_mask,
            ));
            ctx.emit(Inst::xmm_rm_r(
                SseOpcode::Pcmpeqb,
                RegMem::reg(low_mask.to_reg()),
                low_mask,
            ));
            ctx.emit(Inst::xmm_rmi_reg(
                SseOpcode::Psllw,
            ...
            ctx.emit(Inst::xmm_rmi_reg(SseOpcode::Psllw, RegMemImm::imm(4), tmp));
            //let hi = ctx.alloc_tmp(output_ty).only_reg().unwrap();
            ctx.emit(Inst::gen_move(dst, low_mask.to_reg(), types::I8X16));
            ctx.emit(Inst::xmm_rm_r(
                SseOpcode::Pand,
                RegMem::reg(tmp.to_reg()),
                dst,
