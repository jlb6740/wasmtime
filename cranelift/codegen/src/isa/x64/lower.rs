...
fn lower_insn_to_regs<C: LowerCtx<I = Inst>>(
    ...
    match op {
        ...
        Opcode::Vpopcnt => {
            // Reference code here: https://github.com/WebAssembly/simd/pull/379
            ...
            //__m128i low_mask = _mm_set1_epi8 (0 x0f );
            let low_mask = ctx.alloc_tmp(output_ty).only_reg().unwrap();
            ctx.emit(Inst::xmm_rm_r(SseOpcode::Pxor, RegMem::from(low_mask), low_mask));
            ctx.emit(Inst::xmm_rm_r(SseOpcode::Pcmpeqb, RegMem::reg(low_mask.to_reg()), low_mask);
            ctx.emit(Inst::xmm_rm_r(SseOpcode::Psllw, 4, low_mask));
            ctx.emit(Inst::xmm_rm_r(SseOpcode::Psllr, 4, low_mask));
            ctx.emit(Inst::xmm_rm_r(SseOpcode::Psllw, 4, low_mask));

            // __m128i lo = _mm_and_si128 (v, low_mask );
            let lo = ctx.alloc_tmp(output_ty).only_reg().unwrap();
            ctx.emit(Inst::gen_move(lo, low_mask, types::I8x16));
            ctx.emit(Inst::xmm_rm_r(SseOpcode::Pand, lhs, lo));

            // __m128i hi = _mm_and_si128 (_mm_srli_epi16 (v, 4) , low_mask ) ;
            let tmp = ctx.alloc_tmp(output_ty).only_reg().unwrap();
            ctx.emit(Inst::xmm_rm_r(SseOpcode::Psllw, 4, tmp));
            //let hi = ctx.alloc_tmp(output_ty).only_reg().unwrap();
            ctx.emit(Inst::gen_move(dst, low_mask, types::I8x16));
            ctx.emit(Inst::xmm_rm_r(SseOpcode::Pand, tmp, hi));

            //    __m128i cnt1 = _mm_shuffle_epi8 (lookup , lo) ;
            let tmp2 = ctx.alloc_tmp(output_ty).only_reg().unwrap();
            ctx.emit(Inst::xmm_load_const(constant, tmp2, ty));
            ctx.emit(Inst::xmm_rm_r(SseOpcode::Pshufb, tmp2, lo));

            //    __m128i cnt2 = _mm_shuffle_epi8 (lookup , hi) ;
            ctx.emit(Inst::xmm_rm_r(SseOpcode::Pshufb, tmp2, dst));

            //    return _mm_add_epi8 (cnt1 , cnt2 ) ;
            ctx.emit(Inst::xmm_rm_r(SseOpcode::Paddb, tmp2, dst));
        }
        Opcode::Vconst => {
