    ...
    pub fn generate_type(&self) -> Option<String> {
        ...
        match self.location.kind() {
            ...
            Reg(r) => match r.bits() {
                ...
                256 => Some(format!("Ymm<R::{}Ymm>", self.mutability.generate_type())),
                512 => Some(format!("Zmm<R::{}Zmm>", self.mutability.generate_type())),
            ...
            RegMem(rm) => match rm.bits() {
                ...
                256 => Some(format!("YmmMem<R::{}Ymm, R::ReadGpr>", self.mutability.generate_type())),
                512 => Some(format!("ZmmMem<R::{}Zmm, R::ReadGpr>", self.mutability.generate_type())),
...
impl dsl::Location {
    ...
    pub fn generate_to_string(&self, extension: dsl::Extension) -> String {
        ...
        match self {
            ...
            xmm1 | xmm2 | xmm3 | ymm1 | ymm2 | ymm3 | zmm1 | zmm2 | zmm3 | xmm_m128 | ymm_m256 | zmm_m512
    ...
    pub fn generate_fixed_reg(&self) -> Option<&str> {
        ...
        match self {
            ...
            cl => Some("gpr::enc::RCX"),
            imm8 | imm16 | imm32 | r8 | r16 | r32 | r64 | xmm1 | xmm2 | xmm3 | ymm1 | ymm2 | ymm3 | zmm1
            | zmm2 | zmm3 | xmm_m128 | ymm_m256 | zmm_m512 | rm8 | rm16 | rm32 | rm64 | rm128 => None,
        }
    }
}
...
impl dsl::Mutability {
    ...
    pub fn generate_xmm_regalloc_call(&self) -> &str {
        match self {
            ...
            dsl::Mutability::Write => "write_xmm",
        }
    }
}
