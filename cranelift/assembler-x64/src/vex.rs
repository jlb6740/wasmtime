...
impl<R: Registers> VexInstruction<R> {
    ...
    pub fn new(vex: Vex) -> Self {
        ...
        //vexInstruction.prefix = vex.opcodes.prefixes;
        vex_instruction
    }
    ...
    #[inline(always)]
    //pub fn reg(mut self, reg: impl Into<R>) -> Self {
    ...
    //pub fn rm(mut self, reg: impl Into<XmmMem<R, M>>) -> Self {
    ...
    fn b_bit(&self) -> u8 {
        let reg = match &self.rm {
            //XmmMem::Xmm(xmm) => (*xmm).into(),
            XmmMem::Xmm(xmm) => (*xmm).enc(),
            //XmmMem::Mem(Amode::ImmReg { base, .. }) => base.to_real_reg().unwrap().hw_enc(),
    ...
    fn prefix_last_byte(&self) -> u8 {
        ...
        // TODO: No clone here. Probably need to implement into integer for R?
        // What's the difference between implementing into and using the enc function?
        //let vvvv = self.vvvv.clone().map(|r| r.enc()).unwrap_or(0x00);
        let vvvv = self.vvvv.map(|r| r.into()).unwrap_or(0x00);
